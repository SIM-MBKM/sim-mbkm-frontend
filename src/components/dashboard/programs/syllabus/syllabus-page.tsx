...
import { RegistrationSyllabusCard } from "./registration-syllabus-card"
import { SyllabusUploadModal } from "./syllabus-upload-modal"
import { Search, Filter, SlidersHorizontal, Loader2 } from "lucide-react"
import { Input } from "@/components/ui/input"
import { Button } from "@/components/ui/button"
import { useRegistrationSyllabusesByStudent } from "@/lib/api/hooks"
...
// Define RegistrationWithSyllabus interface that matches our data structure
...
export function SyllabusPage() {
  ...
  const [selectedRegistration, setSelectedRegistration] = useState<RegistrationWithSyllabus | null>(null)
  ...
  const { 
    ...
    error,
    refetch
  } = useRegistrationSyllabusesByStudent()
  ...
  const handleUploadClick = (registration: RegistrationWithSyllabus) => {
  ...
  const pendingUploads = registrations.filter((reg) => reg.syllabus_data === null).length
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 mt-20">
        ...
        <p className="text-gray-600">Memuat data silabus...</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="mt-20 border rounded-lg p-6 bg-red-50 border-red-200 text-red-800">
        <h3 className="font-bold mb-2">Error loading syllabus data</h3>
  ...
  return (
    <div className="space-y-6 mt-20">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <h1 className="text-2xl font-bold text-[#003478]">Input Silabus</h1>
        <p className="text-gray-600">Kelola dan unggah silabus nilai untuk program MBKM Anda</p>
      </motion.div>
      ...
      <div className="space-y-4">
        {filteredRegistrations.length > 0 ? (
          filteredRegistrations.map((registration, index) => (
            ...
            >
              <RegistrationSyllabusCard registration={registration} onUploadClick={() => handleUploadClick(registration)} />
            </motion.div>
          ))
        ) : (
          <motion.div
