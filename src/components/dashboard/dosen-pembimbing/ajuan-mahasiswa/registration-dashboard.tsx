...
import { useRegistrationAdvisor, useApproveStudentRegistrations } from "@/lib/api/hooks/use-query-hooks"
...
export function RegistrationDashboard() {
  ...
  const resetFilters = () => {
    const resetValues = {
      ...
      academic_advisor: "",
      lo_validation: "",
      academic_advisor_validation: activeTab === "all" ? "" : activeTab.toUpperCase()
    }
    setTempFilter({
      ...resetValues,
      lo_validation: "", // Empty string in state
      academic_advisor_validation: resetValues.academic_advisor_validation // Keep tab-based value
    })
  ...
  const { 
    ...
    isError: isErrorAll,
    refetch: refetchAll
  } = useRegistrationAdvisor({
    page: 1,
    limit: 100, // Fetch more to get accurate counts
    filter: {
      ...filter,
      lo_validation: "",
      academic_advisor_validation: "" // Always empty to get all registrations
    }
  })
  ...
  const stats = {
    ...
    pending: allRegistrations.filter(reg => reg.academic_advisor_validation === "PENDING").length,
    approved: allRegistrations.filter(reg => reg.academic_advisor_validation === "APPROVED").length,
    rejected: allRegistrations.filter(reg => reg.academic_advisor_validation === "REJECTED").length,
  }
  ...
  useEffect(() => {
    ...
    setFilter(prev => ({
      ...prev,
      academic_advisor_validation: newAdvisorValidation
    }))
  ...
  // Function to check if any filters are active (excluding academic_advisor_validation which is set by tabs)
            ...
            >
              ...
              {hasActiveFilters() && (
                <Badge className="h-5 w-5 p-0 flex items-center justify-center text-[10px] bg-primary ml-1" variant="default">
                  {Object.values(filter).filter(val => val && val !== filter.academic_advisor_validation).length}
                </Badge>
              )}
            </Button>
          </div>
                  ...
                  <CardContent className="p-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          value={tempFilter.academic_advisor_validation || "all"}
                          onValueChange={(value) => handleFilterChange('academic_advisor_validation', value === "all" ? "" : value)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="All statuses" />
                          </SelectTrigger>
