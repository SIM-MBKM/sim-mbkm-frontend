        ...
        <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
          ...
          <div className="mt-4">
            <div className="flex justify-center mb-4">
              <Badge className="px-3 py-1 text-sm bg-green-100 text-green-700 hover:bg-green-200">{syllabus.activity_name}</Badge>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm ml-auto">{syllabus.user_nrp}</span>
              </div>

              <div className="flex items-center gap-2">
                <School className="h-4 w-4 text-slate-500" />
                <span className="text-sm font-medium">Academic Advisor:</span>
                <span className="text-sm ml-auto">{syllabus.academic_advisor_id}</span>
              </div>

              <div className="flex items-center gap-2">
                <Mail className="h-4 w-4 text-slate-500" />
                <span className="text-sm font-medium">Advisor Email:</span>
                <span className="text-sm ml-auto">{syllabus.academic_advisor_email}</span>
              </div>

              <div className="flex items-center gap-2">
                <BookOpen className="h-4 w-4 text-slate-500" />
                <span className="text-sm font-medium">Registration ID:</span>
                <span className="text-sm ml-auto">{syllabus.registration_id}</span>
              </div>

              <div className="flex items-center gap-2">
                <Calendar className="h-4 w-4 text-slate-500" />
                <span className="text-sm font-medium">File ID:</span>
                <span className="text-sm ml-auto truncate max-w-[200px]" title={syllabus.file_storage_id}>
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-5xl h-[80vh] rounded-lg flex flex-col overflow-hidden">
            <div className="p-4 flex justify-between items-center border-b">
              <h3 className="font-semibold">{syllabus.title}</h3>
